...
export default function archetype(Prism) {
  ...
  'expr' : /\b(?:expr[0-9]|E([0-9]|n))\b/,
	'unkeyworded' : /%(?:.*)/,
  'archetype': {
		pattern: /\b(?:archetype|with metadata)\b/,
		alias: ['keyword']
	},
  'builtin': /\b(?:create_contract|make_asset|put_remove|to_container|put|make_map|make_list|blake2b|key_to_address|check_signature|int_to_nat|call_view|slice|mutez_to_nat|fold|get_entrypoint|reverse|require_entrypoint|exec_lambda|apply_lambda|get_some|is_some|left|right|some|none|isempty|length|put|get|transfer|call|emit|prepend|make_operation)\b/,
	'section': {
		pattern: /\b(?:is|no transfer|state is|called by|sourced by|require|fail if|effect|with effect|from|to|when|otherwise|shadow|postcondition|fails)\b/,
		alias: ['keyword']
	},
  'declaration': /\b(?:import|event|constant|enum|states|variable|asset|to big_map|to iterable_big_map|record|as|initial|identified by|initialized with)\b/,
  'entry': {
		pattern: /\b(?:entry|transition|function)\s+\w+/,
		lookbehind: true,
		inside: {
			'keyword': {
        pattern: /\b(?:entry|transition|function)\b/,
      },
			'function-name': {
        pattern: /\b\w+/,
        alias: 'function',
  ...
  'builtin': /\b(?:unit|lambda|contract|big_map|map|set|option|list|int|nat|tez|string|rational|bytes|key|key_hash|address|sapling|signature|date|duration|bool|operation|aggregate|partition|asset_view|asset_key|asset_value|iterable_big_map)\b/,
